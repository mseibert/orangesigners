...
async function generateOGImage(options) {
  ...
  if (backgroundImage) {
    try {
      const imageBuffer = await sharp(backgroundImage.src).resize(width, height, { fit: "cover" }).jpeg({ quality: 80 }).toBuffer();
...
async function generateBlogOGImage(title, description, pubDate, heroImage) {
